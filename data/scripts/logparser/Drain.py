...
class LogParser:
    ...
    def load_data(self):
        ext = self.check_file_extension(self.logName)
        if (ext == '.log'):
            headers, regex = self.generate_logformat_regex(self.log_format)
            self.df_log = self.log_to_dataframe(os.path.join(self.path, self.logName), regex, headers, self.log_format)
        elif (ext == '.csv'):
            headers, regex = self.generate_csvformat_regex(self.log_format)
            self.df_log = self.csv_to_dataframe(os.path.join(self.path, self.logName), regex, headers)
    ...
    def csv_to_dataframe(self, csv_file, regex, headers):
        log_messages = []
        linecount = 0
        cnt = 0
    ...
    def generate_csvformat_regex(self, logformat):
        headers = []
        regex = ''
        for header in logformat.split():
            headers.append(header.strip('<>')) 
            regex += r'(?P<{}>[^,]+),'.format(header.strip('<>'))  
        regex = r'^' + regex[:-1] + '$'  
